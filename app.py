...
def upload_file():
    ...
    # Run dual annealing
    result = dual_annealing(evaluate, bounds=[lat_bounds, lon_bounds], args=(customers_df, service_centers_df),maxiter=3000)
...
def upload_custom_bounds():
    ...
    result = dual_annealing(evaluate, bounds=[(lat_lower, lat_upper), (lon_lower, lon_upper)], args=(customers_df, service_centers_df),maxiter=3000)
    optimal_new_center = np.round(result.x, 6)
    total_existing_distance = customers_df['minimum'].sum()
